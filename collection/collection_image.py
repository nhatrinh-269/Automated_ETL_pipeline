...
betsiboka_coords_wgs84 = ("long_1","lat_1","long_2","lat_2")
resolution = 1

# Date range
year = "year" # example = 2023
start_month = "start_month" # example = 1
end_month = datetime.now().month
start_date = "start_date" # exmaple = 21
...
log(f'Start downloading satellite images from day {start_date}-{start_month}-{year} to day {end_date}-{end_month}-{year}', logfile)
...
for j in range(start_month, end_month + 1):
    ...
    for i in range(start_date, end_date + 1):
        ...
        try:
            ...
            log(f'The image dated {start} was downloaded successfully', logfile)
        except:
            log(f'The image dated {start} failed to download', logfile)  

log('Finish downloading satellite images', logfile) 